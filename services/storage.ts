...
export const authService = {
  ...
  loadUserSession: async (authUser: any): Promise<AppState> => {
    ...
    if (!dbData) {
        ...
        if (currentData.user && (currentData.user.avatarUrl !== avatarUrl && avatarUrl !== '')) {
             const updatedPayload = {
                 ...currentData,
                 user: {
                     ...currentData.user,
                     avatarUrl: avatarUrl || currentData.user.avatarUrl,
                     username: currentData.user.username || name
                 }
             };
    ...
    const user: User = {
        id: authUser.id,
        username: appStateData.user?.username || name || 'Usuário',
        email: authUser.email || '',
        password: '', 
        avatarUrl: appStateData.user?.avatarUrl || avatarUrl || '',
