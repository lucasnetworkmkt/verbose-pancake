            ...
            {Object.values(TimeBlock).map((block) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Tasks List 
                      MOBILE FIX: Remove overflow-y-auto and max-h constraint on mobile to prevent nested scrolling issues. 
                      Desktop keeps scroll if needed.
                  */}
                  <div className={`flex-1 p-2 space-y-2 ${isSelected ? 'min-h-[100px]' : 'min-h-[60px]'} md:overflow-y-auto md:max-h-[300px] md:scrollbar-thin`}>
                    ...
                    {blockTasks.map(task => (
                      <div key={task.id} className="group bg-app-card border border-gray-800 p-2 rounded flex items-start gap-2 hover:border-gray-600 transition-colors">
                        <button 
                          onClick={(e) => { e.stopPropagation(); toggleTask(task.id); }}
                          className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors shrink-0 ${task.isCompleted ? 'bg-app-gold border-app-gold' : 'border-gray-600 hover:border-gray-400'}`}
                        >
                          {task.isCompleted && <Check size={10} className="text-black font-bold"/>}
                        </button>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                             ...
                             <input 
                                ...
                                className="bg-transparent text-[10px] md:text-xs text-gray-500 w-16 outline-none hover:text-white focus:text-app-gold h-4 p-0"
                             />
                          </div>
                        </div>
                        ...
                        <button 
                          ...
                          className="text-gray-700 hover:text-app-red md:opacity-0 md:group-hover:opacity-100 transition-opacity p-1"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
                       ...
                       {isSelected && (
                         <div className="flex items-center gap-1 shrink-0">
                            <input 
                                ...
                                className="bg-black text-[10px] text-gray-400 rounded px-1 border border-gray-800 outline-none w-12 h-7"
                            />
